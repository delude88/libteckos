...
#ifndef teckos_OPTIONAL_LITE_HPP
#define teckos_OPTIONAL_LITE_HPP
...
#define optional_OPTIONAL_teckos   1
#define optional_OPTIONAL_STD      2

// tweak header support:

#ifdef __has_include
# if __has_include(<teckos/optional.tweak.hpp>)
#  include <teckos/optional.tweak.hpp>
# endif
...
# define optional_CONFIG_SELECT_OPTIONAL  ( optional_HAVE_STD_OPTIONAL ? optional_OPTIONAL_STD : optional_OPTIONAL_teckos )
#endif
...
#ifndef teckos_lite_HAVE_IN_PLACE_TYPES
#define teckos_lite_HAVE_IN_PLACE_TYPES  1
...
#define teckos_lite_in_place_t(      T)  std::in_place_t
#define teckos_lite_in_place_type_t( T)  std::in_place_type_t<T>
#define teckos_lite_in_place_index_t(K)  std::in_place_index_t<K>

#define teckos_lite_in_place(      T)    std::in_place_t{}
#define teckos_lite_in_place_type( T)    std::in_place_type_t<T>{}
#define teckos_lite_in_place_index(K)    std::in_place_index_t<K>{}
...
#define teckos_lite_in_place_t(      T)  teckos::in_place_t(&)( teckos::detail::in_place_type_tag<T>  )
#define teckos_lite_in_place_type_t( T)  teckos::in_place_t(&)( teckos::detail::in_place_type_tag<T>  )
#define teckos_lite_in_place_index_t(K)  teckos::in_place_t(&)( teckos::detail::in_place_index_tag<K> )

#define teckos_lite_in_place(      T)    teckos::in_place_type<T>
#define teckos_lite_in_place_type( T)    teckos::in_place_type<T>
#define teckos_lite_in_place_index(K)    teckos::in_place_index<K>

} // namespace teckos

#endif // optional_CPP17_OR_GREATER
#endif // teckos_lite_HAVE_IN_PLACE_TYPES
...
namespace teckos { namespace optional_lite {
...
//    template< typename > friend class optional;
    ...
    template< class... Args >
    storage_t( teckos_lite_in_place_t(T), Args&&... args )
    {
...
public:
    ...
    >
    optional_constexpr explicit optional( teckos_lite_in_place_t(T), Args&&... args )
    : has_value_( true )
    ...
    >
    optional_constexpr explicit optional( teckos_lite_in_place_t(T), std::initializer_list<U> il, Args&&... args )
    : has_value_( true )
    ...
    template< typename U = T
        optional_REQUIRES_T(
            std::is_constructible<T, U&&>::value
            && !std::is_same<typename std20::remove_cvref<U>::type, teckos_lite_in_place_t(U)>::value
    ...
    : has_value_( true )
    , contained( teckos_lite_in_place(T), std::forward<U>( value ) )
    {}
...
{
    return optional<T>( teckos_lite_in_place(T), std::forward<Args>(args)...);
}

template< typename T, typename U, typename... Args >
optional_constexpr optional<T> make_optional( std::initializer_list<U> il, Args&&... args )
{
    return optional<T>( teckos_lite_in_place(T), il, std::forward<Args>(args)...);
}
...
template< class T >
struct hash< teckos::optional<T> >
{
public:
    std::size_t operator()( teckos::optional<T> const & v ) const optional_noexcept
    {
...
#endif // teckos_OPTIONAL_LITE_HPP
